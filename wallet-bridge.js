(function(){
  ...
  function exclusive(fn){
    const run=()=>navigator.locks?navigator.locks.request('adventure-tv-unified-wallet',fn):fn();
  ...
  async function rewardShare(method){
    return exclusive(async()=>{
      ...
      const result=await w.creditStarCoinReward({
        ...
        rewardId,
        proof:{method,url:location.href,title:document.getElementById('nowTitle')?.textContent||'Adventure TV',verification:'DEVICE_LOCAL'}
      });
      render();
      return result;
    });
  ...
  async function handleShare(event){
    ...
    const title=now&&now.textContent&&!now.textContent.includes('Loading')?now.textContent:'Adventure TV';
